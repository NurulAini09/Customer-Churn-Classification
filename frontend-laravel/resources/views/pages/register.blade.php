...
@section('title', 'Daftar - ChurnPredict AI')
  ...
  <main class="grid min-h-screen place-items-center px-5 py-8">
    <section class="w-full max-w-[420px] rounded-md border border-[#D8E2EC] bg-white shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
      <div class="border-b border-[#D8E2EC] px-6 py-5">
        ...
        <h1 class="text-[22px] font-bold text-[#102A43]">Daftar Akun</h1>
        <p class="mt-1 text-[13px] leading-6 text-[#64748B]">Buat akun untuk mengakses dashboard klasifikasi churn.</p>
      </div>

      <div class="p-6">
        @if ($errors->any())
          <div class="mb-4 rounded-md border border-[#e7d599] bg-[#fff9df] px-3 py-2 text-[13px] font-medium text-[#886b14]">
            Pastikan nama, email, dan password sudah benar.
          </div>
        @endif
        ...
        <form method="POST" action="{{ route('register.store') }}" class="space-y-4">
          @csrf
          <label class="block">
            <span class="mb-1.5 block text-xs font-semibold text-[#475569]">Nama</span>
            <span class="flex h-10 items-center gap-2 rounded-md border border-[#D8E2EC] bg-white px-3 transition focus-within:border-[#DB5A8D]">
              <i data-lucide="user" class="h-4 w-4 shrink-0 text-[#64748B]"></i>
              <input name="name" type="text" value="{{ old('name') }}" required autocomplete="name" class="h-full min-w-0 flex-1 border-0 bg-transparent p-0 text-sm font-medium text-[#102A43] outline-none" placeholder="Nama lengkap">
            </span>
          </label>
        ...
        <div class="mt-5 border-t border-[#E5EEF8] pt-4 text-center text-[13px] font-medium">
          <a href="{{ route('login') }}" class="text-[#DB5A8D] transition hover:text-[#C84A7B]">Sudah punya akun? Login</a>
        </div>
      </div>
    </section>
  </main>
