...
@section('content')
  ...
  <div class="sticky top-0 z-20 -mx-5 mb-3 flex flex-col gap-3 bg-[#F3F6FA] px-5 py-3 sm:-mx-6 sm:flex-row sm:items-center sm:justify-between sm:px-6">
    <div>
      ...
      <p class="mt-1 text-[13px] text-[#64748B]">Ringkasan performa klasifikasi churn dan status integrasi sistem.</p>
    </div>
  ...
  <section class="mb-4 grid overflow-hidden rounded-md border border-[#D8E2EC] bg-white md:grid-cols-2 xl:grid-cols-3">
    @foreach ($stats as $index => $stat)
      <div class="{{ $statMeta[$index]['tone'] ?? 'border-[#D8E2EC]' }} {{ $index === 2 ? 'md:border-t md:border-t-[#D8E2EC] xl:border-t-0 xl:border-l xl:border-l-[#D8E2EC]' : '' }} border-b border-l-2 border-r border-[#E5EEF8] bg-white p-4 last:border-r-0 xl:border-b-0">
  ...
  <div class="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_320px]">
    <section class="min-w-0 rounded-md border border-[#D8E2EC] bg-white">
      <div class="flex items-center justify-between border-b border-[#D8E2EC] px-4 py-3">
        <div>
          ...
          <p class="mt-0.5 text-[12px] text-[#64748B]">Data klasifikasi terakhir yang tersimpan.</p>
        </div>
    ...
    <section class="rounded-md border border-[#D8E2EC] bg-white">
      ...
      <div class="divide-y divide-[#E5EEF8] px-4 text-[13px]">
        ...
        <div class="flex items-center justify-between py-3">
          <span class="text-[#64748B]">API Klasifikasi</span>
